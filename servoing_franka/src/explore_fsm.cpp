...
// #include <servoing_franka/grasp_quality.h>
...
cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg){
    if(state == State::INIT){
        state = State :: GO_HOME;
        // state = State :: GOTONEXTPOSE;       //////////////////////////////       REMOVE THIS LINE //////////////////////
        pcl_conversions::toPCL(*cloud_msg, current_cloud);        
        // std::cout<<"Current state is GOHOME"<<std::endl;
...
int main(int argc, char **argv)
{
    ...
    ros::ServiceClient fusecloud_service_client = nh.serviceClient<servoing_franka::fusecloud_service>("fusecloud_service");
    ros::ServiceClient graspquality_service_client = nh.serviceClient<servoing_franka::graspquality_service>("graspquality_service");
    ros::ServiceClient computepca_service_client = nh.serviceClient<servoing_franka::computepca_service>("computepca_service");
    ...
    while(ros::ok())
    {
        ...
        //     w: -0.0116478                        //     w: -9.08005e-05                  
            ...
            // initial_goal_pose.orientation.x =   0.99833 ;
            // initial_goal_pose.orientation.y =   6.45262e-05;
            // initial_goal_pose.orientation.z =   0.0577688;
            // initial_goal_pose.orientation.w =   -3.55704e-06;
                
            Eigen::Quaterniond resultant_quaternion;
            Eigen::Vector3d object_position(0.4d, 0.0d, 0.01d);
            Eigen::Vector3d current_position(initial_goal_pose.position.x, initial_goal_pose.position.y, initial_goal_pose.position.z);
            ...
            // initial_goal_pose = current_pose;
        ...
        if(state == State::SEGMENT){
            ...
            // copyPointCloud(segment_service_srv.request.in_cloud, current_cloud)
            // segment_service_srv.request.in_cloud = current_cloud;
            if (segment_service_client.call(segment_service_srv))
            {
                bool done = false;
                while(!done)
                {
                    // cloud_object = segment_service_srv.response.status;
            ...
            pcl::fromPCLPointCloud2(cloud_object, *point_cloud_ptr); //* convert from pcl/PCLPointCloud2 to pcl::PointCloud<T>
        ...
        if(state == State::FUSE){
            ...
            if (fusecloud_service_client.call(fusecloud_service_srv))
            {
                ...
                while(!done)
                {
                    // cloud_object = fusecloud_service_srv.response.status;
            ...
            pcl::PointCloud<pcl::PointXYZRGB>::Ptr point_cloud_ptr (new pcl::PointCloud<pcl::PointXYZRGB>);
            pcl::fromPCLPointCloud2 (fused_cloud_object, *point_cloud_ptr); //* convert from pcl/PCLPointCloud2 to pcl::PointCloud<T>
        ...
        if(state == State::CHECK_GRASPQUALITY){
            ...
            if (graspquality_service_client.call(graspquality_service_srv))
            {
                ...
                while(!done)
                {
                    // cloud_object = graspquality_service_srv.response.status;
                    // pcl_conversions::toPCL(graspquality_service_srv.response.fused_cloud, fused_cloud_object);
                ...
                if(graspquality_service_srv.response.found){
                    ...
                    if((abs(normal_distvector_angle - 180) < NORMAL_ANGLE_TH) && abs(normals_angle - 180) < NORMAL_ANGLE_TH){
                        std::cout<<"----------------FOUND AT------" << normal_distvector_angle <<"and" <<normals_angle <<std::endl;
